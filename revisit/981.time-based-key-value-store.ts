...
class TimeMap {
    ...
    constructor() {
        this.data = new Map<string, dataType[]>();
    }

    set(key: string, value: string, timestamp: number): void {
        const found = this.data.get(key)
        if (found) {
            found.push({value, timestamp})
        } else{
            this.data.set(key, [{value, timestamp}])
        }
    }

    binarySearch(dataArr: dataType[], target: number) {
        if (dataArr.length === 0) return -1
        let l = 0, r = dataArr.length - 1
        while (l <= r) {
            const m = Math.floor((r- l) / 2) + l;
            if (dataArr[m].timestamp <= target) {
                l = m + 1
            } else {
                r = m - 1
            }
    ...
    get(key: string, timestamp: number): string {
        const valArr = this.data.get(key);
        if (valArr === undefined || valArr.length === 0)  return ""

        const index = this.binarySearch(valArr, timestamp)
        if (index === -1) {
            return "";
        }
        return valArr[index].value
    }
}
