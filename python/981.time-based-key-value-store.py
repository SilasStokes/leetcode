...
class TimeMap:
    ...
    def set(self, key: str, value: str, timestamp: int) -> None:
      if key in self.map.keys():
        # all of the values of timestamp are strictly increasing
        self.map[key].append({'val': value, 'timestamp': timestamp})
      else:
        self.map[key] = [ {'val': value, 'timestamp' : timestamp}]
    ...
    def get(self, key: str, timestamp: int) -> str:
      ...
      val_map = self.map[key]
      l, r = 0, len(val_map) - 1
      while l <= r:
        i = (r - l) // 2 + l

        if timestamp == val_map[i]['timestamp']:
          return val_map[i]['val']
        
        if val_map[i]['timestamp'] < timestamp:
          res = val_map[i]['val']
          l = i + 1
        else:
          r = i - 1
